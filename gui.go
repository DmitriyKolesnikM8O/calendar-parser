...
func RunGUI() {
	...
	submitButton.OnTapped = func() {
		...
		for colorID, timeRanges := range eventsColorTime {
			...
			resultText += fmt.Sprintf("Total time for %s: %s\n", color, formatHours(totalHours))
		}

		resultLabel.SetText(resultText)

		statistics(eventsColorTime, dateFrom.Text, dateTo.Text)
	...
	viewChatButton.OnTapped = func() {
		...
		switch runtime.GOOS {
		case "windows":
			cmd = exec.Command("start", "bar.html")
		case "darwin":
			cmd = exec.Command("open", "bar.html")
		default:
			cmd = exec.Command("xdg-open", "bar.html")
		}
